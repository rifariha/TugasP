...
class Transaction:
    ...
    def import_data(self, df):
        ...
        conns.create_database(conn)
        conns.create_table(conn)

        cursor = conns.select_database(conn, "football_player")
        ...
        if jumlah <= 0:
            ...
            for i in df.index:
                cursor.execute(sql,
                               (df['name'][i], df['age'][i], df['nationality'][i], df['born_year'][i], df['squad'][i]))
                conn.commit()
    ...
    def insert_data(self, name, age, nationality, born_year, squad):
        conns = connection.Connection()
    ...
    def insert_new_data(self):
        ...
        born_year = input("Player born year: ")
        squad = input("Player squad: ")

        self.insert_data(name, age, nationality, born_year, squad)
    ...
    def show_data(self, val):
        ...
        if val == "1":
            ...
            operator_option = self.select_operator()
            ...
            if operator_option == "1":
                sql = "select * from players where {field} = '{value}' order by {sort} {sort_option}".format(
                    field=field,
                    value=value,
                    sort=sortBy,
